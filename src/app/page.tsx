    ...
    <div className="flex gap-4 flex-wrap">
      {SERIES.map((s) => (
        ...
        >
          ...
          />
          <span className={visible[s.key] ? "text-gray-700" : "text-gray-400 line-through decoration-gray-300"}>
            {s.key}
          </span>
        </button>
      ))}
...
function PeriodPicker({
  ...
  return (
    <div className="relative" ref={wrapRef}>
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className={`flex items-center gap-2.5 px-4 py-2.5 bg-white rounded-xl border transition-all duration-150 ${
          open
            ? "border-indigo-300 shadow-md shadow-indigo-100/60"
            : "border-gray-200 shadow-sm hover:border-indigo-200 hover:shadow-md hover:shadow-indigo-50"
        }`}
      >
        {/* Calendar SVG */}
        <svg className={`w-4 h-4 shrink-0 transition-colors ${open ? "text-indigo-500" : "text-gray-400"}`}
        ...
        <div className="text-left">
          <span className={`block text-[10px] font-semibold leading-none mb-0.5 uppercase tracking-wider transition-colors ${
            open ? "text-indigo-400" : "text-gray-400"
          }`}>{modeLabel}</span>
          <span className="block text-sm font-bold text-gray-800 leading-tight whitespace-nowrap">{triggerLabel}</span>
        </div>

        {/* Chevron */}
        <svg className={`w-3.5 h-3.5 text-gray-400 shrink-0 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
      ...
      {open && (
        <div
          className="absolute right-0 top-[calc(100%+8px)] z-50 bg-white rounded-2xl overflow-hidden animate-picker-enter"
          style={{
            width: 456,
            boxShadow: "0 20px 60px -10px rgba(0,0,0,0.14), 0 4px 20px -4px rgba(99,102,241,0.08)",
            border: "1px solid rgba(0,0,0,0.06)",
          }}
        >
          {/* ── Panel header: year navigator ───────────────── */}
          {mode !== "yearly" && (
            <div className="flex items-center px-4 py-2.5 bg-gray-50/80 border-b border-gray-100/80">
              {mode === "weekly" && weekStep === "week" ? (
                ...
                <>
                  <button
                    ...
                    className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all font-bold text-base leading-none"
                  >‹</button>
                  <span className="flex-1 text-center text-sm font-bold text-gray-800">{navYear}</span>
                  <button
                    onClick={() => setNavYear((y) => y + 1)}
                    className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all font-bold text-base leading-none"
                  >›</button>
          ...
          {mode === "yearly" && (
            <div className="px-5 py-2.5 bg-gray-50/80 border-b border-gray-100/80">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pilih Tahun</p>
            </div>
          )}
          ...
          <div className="flex">
            ...
            {/* Left sidebar */}
            <div className="w-[168px] shrink-0 border-r border-gray-100/80 py-1.5">
              {MODES.map(({ id, label }) => (
                <button
                  key={id}
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 text-sm transition-all duration-150 border-r-2 ${
                    mode === id
                      ? "text-indigo-600 font-semibold bg-indigo-50/70 border-indigo-500"
                      : "text-gray-600 font-medium hover:bg-gray-50/80 hover:text-gray-800 border-transparent"
                  }`}
                >
                  <span>{label}</span>
                  {id !== "yearly" && (
            ...
            <div className="flex-1 p-4">
              ...
              {mode === "weekly" && weekStep === "month" && (
                <div className="animate-slide-left">
                  <p className="text-[11px] text-gray-400 font-medium mb-3 uppercase tracking-wide">
                    Pilih Bulan
                  </p>
              ...
              {mode === "yearly" && (
                <div className="space-y-1.5 animate-slide-left">
                  {availableYears.length > 0 ? availableYears.map((y) => (
                    ...
                    <p className="text-sm text-gray-400 text-center py-6">Belum ada data tahun</p>
                  )}
                </div>
              )}
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex gap-3 items-start">
      ...
      <div className="min-w-0">
        ...
        <p className="text-lg font-bold text-gray-900 mt-0.5 leading-tight tabular-nums">{value}</p>
...
function FinCard({ label, value, sub, highlight, valueColor, curNum, prevNum, higherIsBetter = true, fmtFn }: {
  ...
  return (
    <div className={`rounded-xl p-3 ${highlight ? "bg-indigo-50 border border-indigo-100" : "bg-gray-50 border border-gray-100"}`}>
      <p className="text-xs text-gray-500 mb-1">{label}</p>
      <p className={`text-sm font-bold tabular-nums ${valueColor ?? (highlight ? "text-indigo-700" : "text-gray-800")}`}>{value}</p>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline justify-between gap-2 mb-1">
          <p className="text-sm leading-tight min-w-0 truncate">
            ...
            {visualTake && (
              <span className="text-gray-400 font-normal ml-1">· {visualTake}</span>
            )}
          </p>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Affiliate Analytics · Asterixsty Perfumery</p>
        </div>
      ...
      {isEmpty ? (
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-20 text-center">
          ...
          <h3 className="font-semibold text-gray-700 mb-1">Belum ada data</h3>
        ...
        <>
          ...
          {data?.financial && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-bold text-gray-800">💼 Financial Overview</h2>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
            ...
            <div className="xl:col-span-3 bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
              <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
                <div>
                  <h2 className="text-sm font-bold text-gray-800">📈 Trend GMV {data?.selectedYear}</h2>
                  <p className="text-xs text-gray-400 mt-0.5">Data bulanan sepanjang tahun</p>
                </div>
            ...
            {/* Top 10 — takes 2/5 */}
            <div className="xl:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
              <div className="mb-4">
                <h2 className="text-sm font-bold text-gray-800">🏆 Top 10 Affiliate</h2>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
            ...
            {(data?.hallOfFame ?? []).length > 0 && (
              <div className="xl:col-span-3 bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
                <div className="mb-4">
                  <h2 className="text-sm font-bold text-gray-800">🏅 Hall of Fame</h2>
                  <p className="text-xs text-gray-400 mt-0.5">Kumulatif lifetime GMV semua periode</p>
                </div>
            ...
            <div className={`${(data?.hallOfFame ?? []).length > 0 ? "xl:col-span-2" : "xl:col-span-5"} bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4`}>
              <div className="mb-4">
                <h2 className="text-sm font-bold text-gray-800">👥 Creator Aktif</h2>
                <p className="text-xs text-gray-400 mt-0.5">Trend per bulan · {data?.selectedYear}</p>
              </div>
